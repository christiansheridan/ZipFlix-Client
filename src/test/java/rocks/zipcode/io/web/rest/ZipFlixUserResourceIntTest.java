...
public class ZipFlixUserResourceIntTest {
     ...
     */
    public static ZipFlixUser createEntity(EntityManager em) {
    ...
    public void getZipFlixUser() throws Exception {
        ...
        restZipFlixUserMockMvc.perform(get("/api/zip-flix-users/{id}", zipFlixUser.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(zipFlixUser.getId().intValue()))
    ...
    public void updateZipFlixUser() throws Exception {
        ...
        // Disconnect from session so that the updates on updatedZipFlixUser are not directly saved in db
        em.detach(updatedZipFlixUser);
        updatedZipFlixUser
